...
import { logger } from "https://esm.sh/hono@4/logger";
...
// Root route for quick checks
app.get("/", (c) => c.json({ status: "ok", root: true }));
...
app.get("/submissions/my", async (c) => {
  const { user, error } = await verifyAuth(c.req.header('Authorization'));
  if (error) return c.json({ error }, 401);
...
app.get("/monitoring/active", async (c) => {
  const { user, error } = await verifyAuth(c.req.header('Authorization'));
  if (error) return c.json({ error }, 401);
